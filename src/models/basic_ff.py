...
class BasicFeedForward(nn.Module):
    ...
    def forward(self, x):
        ...
        tripID_embedded = self.tripID_em(x[:,self.embed_dict['tripID']['col']].int())
        ...
        x = torch.cat([continuous_X, timeID_embedded, weekID_embedded, driverID_embedded, tripID_embedded], dim=1)
        # Make prediction
